# Copied from https://github.com/xinge008/Cylinder3D/blob/cc2116c8cb99b8c23c186691f344b1d95ee0d04f/utils/lovasz_losses.py#L156
...
def hinge_jaccard_loss(probas, labels, ignore=None, classes='present', hinge=0.1, smooth=100):
    ...
    for c in class_to_sum:
        if c in vlabels:
            ...
            TP = torch.sum(torch.clamp(
                class_pred - max_non_class_pred, max=hinge)+1.) + smooth
            FN = torch.sum(torch.clamp(max_non_class_pred -
                           class_pred, min=-hinge)+hinge)
            ...
            if (~c_sample_ind).sum() == 0:
                ...
                max_non_class_pred = torch.max(
                    nonc_probas[:, non_c_ind], dim=1)[0]
                FP = torch.sum(torch.clamp(
                    class_pred - max_non_class_pred, max=hinge)+1.)
