...
from pids_core.models.blocks import PointOperator
from pids_core.models.losses import get_l2_loss
...
def step_drop_connect(m):
    if isinstance(m, PointOperator):
        m.step_drop_connect(False)

def get_lr_scheduler(optimizer, lr_scheduler='cosine', num_epochs=90):
    if lr_scheduler == "multistep":
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [num_epochs//3, num_epochs*2//3],
                                                            gamma=0.1)
    elif lr_scheduler == "multistep-search":
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [int(num_epochs * .6), int(num_epochs * .8)],
                                                            gamma=0.5)
    elif lr_scheduler == 'none':
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, [num_epochs * 3, num_epochs * 6],
                                                            gamma=0.1)
    elif lr_scheduler == "cosine":
        lr_scheduler = CosineAnnealingLRWarmup(optimizer, T_max=num_epochs, last_epoch=-1, eta_min=1e-5)
    elif lr_scheduler == 'step':
        lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer,
                                                            [int(num_epochs*0.6), int(num_epochs*0.75), int(num_epochs*0.9)],
                                                            gamma=0.1)
...
VALIDATE_EVERY_K_EPOCH = 2
class ModelTrainer:
    # Initialization methods
    # ------------------------------------------------------------------------------------------------------------------
    def __init__(self, net, config, chkp_path=None, finetune=False, on_gpu=True, optimizer='sgd'):
        """
        ...
        if config.optimizer == 'sgd':
            ...
            self.optimizer = torch.optim.Adam([{'params': other_params},
                                              ...
                                              weight_decay=0,
                                              eps=1e-2)
        elif config.optimizer == 'rmsprop-tf':
            self.optimizer = RMSpropTF([{'params': other_params},
                                        {'params': deform_params, 'lr': deform_lr}],
                                        lr=config.learning_rate,
                                        momentum=config.momentum,
                                        weight_decay=0,
    ...
    def train(self, net, training_loader, val_loader, config):
        ...
        lr_scheduler = get_lr_scheduler(self.optimizer, config.lr_schedule, config.max_epoch)
        
        # Wrap network with EMA if needed.
        if config.ema != 0:
            print("Wrapping network object with EMA={:f}".format(config.ema))
            net = EMA(net, config.ema)
            loss_fn = net.model.loss
            accuracy_fn = net.model.accuracy
        else:
            loss_fn = net.loss
            accuracy_fn = net.accuracy

        best_val_perf = 0.
        net.train()
        # Important: adjust init_scale for the best performance.
        if config.amp:
            print("Enable AMP!")
        ...
        for epoch in range(config.max_epoch):
            ...
            for idx, batch in enumerate(training_loader):
                ...
                with autocast(enabled=enable_amp):
                    ...
                    l2_loss = get_l2_loss(net, config.weight_decay)
                    total_loss = loss + l2_loss

                acc = accuracy_fn(outputs, batch.labels)
                ...
                if (t[-1] - last_display) > 5.0:
                    last_display = t[-1]
                    message = 'e{:03d}-i{:04d} => CE_L={:.3f} L2={:.3f} acc={:3.0f}% / t(ms): {:5.1f} {:5.1f} {:5.1f})'
                ...
                if config.saving:
                    if config.ema == 0:
                        output_loss_txt = net.output_loss
                        reg_loss_txt = net.reg_loss
                    else:
                        output_loss_txt = net.model.output_loss
                        reg_loss_txt = net.model.reg_loss
            ...
            if self.epoch in config.lr_decays:
                ...
                print("Learning rate updated to %.5f" %max_lr)
                """
            # Update epoch
            self.epoch += 1
            ...
            perf = 0.
            net.eval()
            if not config.no_validation and epoch % VALIDATE_EVERY_K_EPOCH == (VALIDATE_EVERY_K_EPOCH-1):
                with torch.no_grad():
                    perf = self.validation(net, val_loader, config)
            # Saving
            if perf != 0 and config.saving and not config.no_validation or (config.no_validation and epoch == config.max_epoch-1):
                ...
                if perf > best_val_perf or (config.no_validation and epoch == config.max_epoch-1):
                    checkpoint_path = join(checkpoint_directory, 'chkp_best.tar')
                    torch.save(save_dict, checkpoint_path)                    
                    if not (config.no_validation and epoch == config.max_epoch-1):
                        print("Best perf: {:.4f} --> {:.4f}".format(best_val_perf, perf))
                    best_val_perf = perf
                # Save current state of the network (for restoring purposes)
                checkpoint_path = join(checkpoint_directory, 'current_chkp.tar')
                torch.save(save_dict, checkpoint_path)
                # Save checkpoints occasionally
                if (self.epoch + 1) % config.checkpoint_gap == 0 or self.epoch == config.max_epoch-1:
    ...
    def object_classification_validation(self, net, val_loader, config):
        ...
        cur_validation_step = 0

        while cur_validation_step < config.validation_size:
            probs = []
            targets = []
            obj_inds = []
            t = [time.time()]
            ...
            for batch in val_loader:
                ...
                if (t[-1] - last_display) > 1.0:
                    last_display = t[-1]
                    message = 'Validation {:d}: {:.1f}% (timings : {:4.2f} {:4.2f})'
                    print(message.format(cur_validation_step, 100 * len(obj_inds) / config.validation_size,
                                        1000 * (mean_dt[0]),
                                        1000 * (mean_dt[1])))
                if cur_validation_step >= config.validation_size:
                    break
