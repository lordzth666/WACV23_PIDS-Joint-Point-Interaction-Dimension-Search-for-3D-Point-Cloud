...
class Config:
    ...
    # Assign a dropout rate.
    dropout = 0.0
    drop_connect = 0.0
    # Add the label smoothing factor
    label_smoothing = 0.0
    ...
    # Disable validation for proxy training
    no_validation = False
    num_votes = 100
    ...
    # Test smoothing.
    test_smooth = 0.9
    # Additional FLAGS: Attention involved.
    use_interblock_attn = False
    use_enc_dec_attn = False
    ...
    def load(self, path):
        ...
        for line in lines:
            ...
            if len(line_info) > 2 and line_info[0] != '#':
                ...
                elif line_info[0] == 'lr_schedule':
                    self.lr_schedule = line_info[2]
                ...
                elif line_info[0] == 's3dis_validation_split':
                    self.s3dis_validation_split = int(line_info[2])

                elif line_info[0] == 'use_interblock_attn':
                    self.use_interblock_attn = eval(line_info[2])

                elif line_info[0] == 'use_enc_dec_attn':
                    self.use_enc_dec_attn = eval(line_info[2])
    ...
    def save(self):
        ...
        with open(join(self.saving_path, 'parameters.txt'), "w") as text_file:
            ...
            text_file.write('stem_conv_dim = {:d}\n'.format(self.stem_conv_dim))
            text_file.write('head_conv_dim = {:d}\n'.format(self.head_conv_dim))
            ...
            text_file.write('use_interblock_attn = {}\n'.format(self.use_interblock_attn))
            text_file.write('use_enc_dec_attn = {}\n'.format(self.use_enc_dec_attn))
            ...
            text_file.write("translate_scale = {:.6f}\n\n".format(self.translate_scale))

            text_file.write('weight_decay = {:f}\n'.format(self.weight_decay))
            text_file.write('dropout = {:f}\n'.format(self.dropout))
            text_file.write('drop_connect = {:f}\n'.format(self.drop_connect))
            text_file.write('label_smoothing_factor = {:f}\n'.format(self.label_smoothing))
            ...
            if hasattr(self, 's3dis_validation_split'):
                text_file.write("s3dis_validation_split = {:d}\n".format(self.s3dis_validation_split))
