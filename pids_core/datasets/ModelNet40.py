...
from pids_core.datasets.common import PointCloudDataset
...
class ModelNet40Dataset(PointCloudDataset):
    ...
    def __init__(self, config, split="train", orient_correction=True):
        """
        ...
        # Dataset folder
        self.path = './data/modelnet40_normal_resampled'
        ...
        if self.split == "train":
            ...
            self.epoch_n = min(self.num_models, config.epoch_steps * config.batch_num)
        elif self.split == "mini-train":
            self.num_models = 7874
            self.epoch_n = min(self.num_models, config.epoch_steps * config.batch_num)            
        elif self.split == "mini-val":
            self.num_models = 1969
    ...
    def load_subsampled_clouds(self, orient_correction):
        ...
        print('\nLoading {:s} points subsampled at {:.3f}'.format(self.split, self.config.first_subsampling_dl))
        filename = join(self.path, '{:s}_{:.3f}_record.pkl'.format(self.split, self.config.first_subsampling_dl))
        ...
        # Else compute them from original points
            ...
            name_target_files_list = {
                'train': join(self.path, 'modelnet40_train.txt'),
                'mini-train': join(self.path, 'modelnet40_mini_train.txt'),
                'mini-val': join(self.path, 'modelnet40_mini_val.txt'),
                'test': join(self.path, 'modelnet40_test.txt'),
            }

            names = np.loadtxt(name_target_files_list[self.split], dtype=np.str)
