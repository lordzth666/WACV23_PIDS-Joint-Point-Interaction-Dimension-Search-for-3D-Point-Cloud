...
from pids_core.datasets.common import *
from pids_core.utils.mayavi_visu import *
from pids_core.utils.metrics import fast_confusion

from pids_core.datasets.common import grid_subsampling
from pids_core.utils.config import bcolors
# ----------------------------------------------------------------------------------------------------------------------
#
...
class SemanticKittiDataset(PointCloudDataset):
    ...
    def __init__(self, config, set='training', balance_classes=True):
        ...
        # Dataset folder
        self.path = './data/semantic_kitti/dataset'
        ...
        if self.set == 'training':
            ...
            input("Training on validation dataset... This is only used for submission purpose. "
                  "Press [Enter] to continue")
        elif self.set == 'mini-train':
            self.sequences = ['{:02d}'.format(i) for i in range(3)]
        elif self.set == 'mini-val':
            self.sequences = ['{:02d}'.format(6)]
        ...
        if self.set == 'training' or self.set == "trainval" or self.set == "mini-train":
        ...
        if set == 'training' or self.set == "trainval" or self.set == "mini-train":
    ...
    def __getitem__(self, batch_i):
        ...
        while True:
            ...
            while num_merged < self.config.n_frames and f_ind - f_inc >= 0:
                ...
                if self.set in ['validation', 'test', 'mini-val'] and f_inc == 0:
            ...
            if self.set in ['validation', 'test', 'mini-val']:
    ...
    def load_calib_poses(self):
        ...
        if self.set in ['training', 'validation', 'trainval', 'mini-train', 'mini-val']:
        ...
        if self.set == 'validation' or self.set == "mini-val":
            self.val_points = []
            self.val_labels = []
            self.val_confs = []
...
class SemanticKittiSampler(Sampler):
    ...
    def __init__(self, dataset: SemanticKittiDataset):
        ...
        if dataset.set == 'training' or dataset.set == "trainval" or dataset.set == "mini-train":
        ...
        if self.dataset.balance_classes:
            ...
            for i, c in enumerate(self.dataset.label_values):
                if c not in self.dataset.ignored_labels:
                    ...
                    self.dataset.potentials[update_inds] += torch.rand(update_inds.shape[0]) * 0.1 + 0.1
            ...
            # Update potentials (Change the order for the next epoch)
            self.dataset.potentials[gen_indices] = torch.ceil(self.dataset.potentials[gen_indices])
            self.dataset.potentials[gen_indices] += torch.rand(gen_indices.shape[0]) * 0.1 + 0.1
    ...
    def calib_max_in(self, config, dataloader, untouched_ratio=0.9, verbose=True, force_redo=False):
        """
        ...
        print("Untouched ratio: {:.5f}".format(untouched_ratio))
        t0 = time.time()
        ...
        if self.dataset.set in ['mini-train', 'mini-val']:
            max_in_limits_pkl = "max_in_limits_search.pkl"
        elif self.dataset.set in ['test']:
            max_in_limits_pkl = "max_in_limits_test.pkl"
        else:
            max_in_limits_pkl = "max_in_limits.pkl"
        max_in_lim_file = join(self.dataset.path, max_in_limits_pkl)
        ...
        if self.dataset.set == 'training' or self.dataset.set == "trainval":
    ...
    def calibration(self, dataloader, untouched_ratio=0.9, verbose=False, force_redo=False):
        ...
        if self.dataset.set in ['mini-train', 'mini-val']:
            batch_limits_pkl = "batch_limits_search.pkl"
        elif self.dataset.set in ['test']:
            batch_limits_pkl = "batch_limits_test.pkl"
        else:
            batch_limits_pkl = "batch_limits.pkl"
        batch_lim_file = join(self.dataset.path, batch_limits_pkl)
        ...
        neighb_limits_pkl = "neighbors_limits_search.pkl" \
            if dataloader.dataset.set in ['mini-train', 'mini-val'] else "neighbors_limits.pkl"
        neighb_lim_file = join(self.dataset.path, neighb_limits_pkl)
...
def gen_random_neighbors(x: np.ndarray):
    np.random.seed(233)
    all_random_neighbors = np.random.randint(0, x.shape[0], size=[x.shape[0], x.shape[1]])
    np.random.seed(None)
    return torch.from_numpy(all_random_neighbors)
...
class SemanticKittiCustomBatch:
    ...
    def __init__(self, input_list):
        ...
        # maniuplate neighbors to be completely random.
        # self.neighbors: List[torch.Tensor] = [gen_random_neighbors(x) for x in input_list[ind:ind+L]]
        ind += L
