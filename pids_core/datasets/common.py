...
from pids_core.kernels.kernel_points import create_3D_rotations

# Subsampling extension
import pids_core.cpp_wrappers.cpp_subsampling.grid_subsampling as cpp_subsampling
import pids_core.cpp_wrappers.cpp_neighbors.radius_neighbors as cpp_neighbors
...
def batch_neighbors(queries, supports, q_batches, s_batches, radius):
    ...
    """
    out = cpp_neighbors.batch_query(queries, supports, q_batches, s_batches, radius=radius)
    return out
...
class PointCloudDataset(Dataset):
    ...
    def augmentation_transform(self, points, normals=None, verbose=False):
        ...
        # Add translate here.
        if self.config.translate_scale != 0:
            translate_xyz = np.random.uniform(-self.config.translate_scale, -self.config.translate_scale, size=points.shape[1])
            augmented_points += translate_xyz
